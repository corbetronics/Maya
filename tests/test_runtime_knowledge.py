...
def test_admin_runtime_upload_accepts_valid_index(tmp_path: Path, monkeypatch) -> None:
    """Confirm admin upload writes and reloads a valid runtime index."""
    upload_path = tmp_path / "runtime" / "runtime_index.json"
    runtime_index = runtime_index_payload()
    monkeypatch.setenv("MAYA_ADMIN_UPLOAD_TOKEN", "upload-token")
    monkeypatch.setattr("backend.main.RUNTIME_INDEX_UPLOAD_PATH", upload_path)
    monkeypatch.setenv("MAYA_KNOWLEDGE_INDEX_PATH", str(upload_path))
    client = TestClient(create_app())

    response = client.post(
        "/maya/admin/upload-runtime-index",
        content=json.dumps(runtime_index),
        headers={
            "Content-Type": "application/json",
            "X-Maya-Admin-Token": "upload-token",
        },
    )
    ...
    assert payload["file_size"] == upload_path.stat().st_size
    assert client.get("/maya/knowledge-status").json()["knowledge_loaded"] is True


def test_admin_runtime_upload_rejects_bad_token(tmp_path: Path, monkeypatch) -> None:
    """Confirm upload requires the private token header."""
    monkeypatch.setenv("MAYA_ADMIN_UPLOAD_TOKEN", "upload-token")
    monkeypatch.setattr("backend.main.RUNTIME_INDEX_UPLOAD_PATH", tmp_path / "runtime_index.json")
    client = TestClient(create_app())

    response = client.post(
        "/maya/admin/upload-runtime-index",
        content=json.dumps(runtime_index_payload()),
        headers={"X-Maya-Admin-Token": "wrong-token"},
    )

    assert response.status_code == 401


def test_admin_runtime_upload_rejects_oversized_file(tmp_path: Path, monkeypatch) -> None:
    """Confirm files over the 2 MB limit are rejected before parsing."""
    monkeypatch.setenv("MAYA_ADMIN_UPLOAD_TOKEN", "upload-token")
    monkeypatch.setattr("backend.main.RUNTIME_INDEX_UPLOAD_PATH", tmp_path / "runtime_index.json")
    client = TestClient(create_app())
    ...
    response = client.post(
        "/maya/admin/upload-runtime-index",
        content=b"{" + (b" " * (2 * 1024 * 1024 + 1)) + b"}",
        headers={"X-Maya-Admin-Token": "upload-token"},
    )

    assert response.status_code == 413


def test_admin_runtime_upload_rejects_invalid_json(tmp_path: Path, monkeypatch) -> None:
    """Confirm invalid JSON does not write the runtime index."""
    upload_path = tmp_path / "runtime_index.json"
    monkeypatch.setenv("MAYA_ADMIN_UPLOAD_TOKEN", "upload-token")
    monkeypatch.setattr("backend.main.RUNTIME_INDEX_UPLOAD_PATH", upload_path)
    client = TestClient(create_app())
...
def test_admin_runtime_upload_rejects_forbidden_fields(tmp_path: Path, monkeypatch) -> None:
    """Confirm raw transcript/audio/debug fields cannot enter runtime storage."""
    payload = runtime_index_payload()
    payload["episodes"][0]["chunks"][0]["source_transcript_path"] = "transcripts/episode.txt"
    monkeypatch.setenv("MAYA_ADMIN_UPLOAD_TOKEN", "upload-token")
    monkeypatch.setattr("backend.main.RUNTIME_INDEX_UPLOAD_PATH", tmp_path / "runtime_index.json")
    client = TestClient(create_app())
...
def runtime_index_payload() -> dict[str, object]:
    """Return a valid compact runtime index fixture."""
    return {
        "schema_version": 1,
                ...
                "chunks": [
                    {
                        ...
                        "title": "289: Nothing Has Rocked My World Like the Walkman",
                        "publication_date": "2026-06-17",
                        "chunk_id": "episode-289-walkman-0",
